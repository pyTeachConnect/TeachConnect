...
if weekday == 6:
    notification.notify(
        title="升级提醒",
        message=f"请将 TeachConect 客户端更新到最新版本！GitHub仓库：{html_path}",
        timeout=5  # 通知显示的时长
    )
...
class LoginDialog(QDialog):
    ...
    def register_user(self):
        if self.register_button.text() == "已注册":
            reply = QMessageBox.question(self, '警告', '已经注册过账户，确认继续注册？', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes:
                ...
                if username in users:
                    ...
                    # self.register_button.setEnabled(False)  # 允许创建新用户
                    self.register_button.setText("已注册")  # 显示“已注册”文字
            else:
                exit()
        else:
